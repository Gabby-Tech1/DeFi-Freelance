...
actor Freelance {
    ...
    public query func listJobs() : async [Job] {
        Array.freeze(HashMap.toArray<Text, Job>(jobs))
    };
    ...
    private func getAllUserRatings(userId: Principal) : [Rating] {
        Array.filter<Rating>(
            Array.freeze(HashMap.toArray<Text, Rating>(ratings)),
            ...
            case (#ICRC1(canisterId)) {
                try {
                    ...
                    switch (result) {
                        ...
                        case (#Err(e)) {
                            switch (e) {
                                case (#InsufficientFunds(_)) #err("Insufficient funds");
                                case (#BadFee(_)) #err("Invalid fee");
                                case (#BadBurn(_)) #err("Invalid burn");
                                case (#Other(msg)) #err(msg);
                            }
                        };
                    }
                } catch (e) {
